...
func TestAPI_DiffSemaphoreThrottling(t *testing.T) {
	db, err := InitDB(fmt.Sprintf("file:mem_sem_%d?mode=memory&cache=shared", time.Now().UnixNano()))
	if err != nil {
		t.Fatalf("failed to init database: %v", err)
	}
	defer db.Close()
	...
	adminToken, _ := GenerateToken("admin", "admin", "sem-secret-key-12345", 5*time.Second)
