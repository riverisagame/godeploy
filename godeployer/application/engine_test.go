...
func TestEngine_Scheduler_ConcurrencyLimit(t *testing.T) {
	...
	for i := 0; i < 60; i++ {
		err := engine.SubmitJob(&domain.DeployJob{
			TaskID: int64(i),
		})
		if err == ErrQueueFull {
			failCount++
...
func TestEngine_Scheduler_GracefulShutdown(t *testing.T) {
	...
	// 测试停机后是否拒绝新的提交
	err = engine.SubmitJob(&domain.DeployJob{TaskID: 1000})
		...
		Projects: map[string]domain.ProjectConfig{
			"test-proj": {
				...
				Environments: []domain.EnvironmentConfig{
					{
						ID:      "prod",
						Servers: []domain.ServerConfig{{Host: "localhost", DeployTo: "/opt/app"}},
					},
				},
			},
		},
	...
	config := &domain.Config{
		Projects: map[string]domain.ProjectConfig{
			"concurrent-proj": {
				...
				Repo: repoDir,
				Environments: []domain.EnvironmentConfig{
					{ID: "prod", Servers: []domain.ServerConfig{{Host: "localhost", DeployTo: "/opt/app"}}},
				},
			},
		},
	}
