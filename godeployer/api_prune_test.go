...
func TestAPI_SystemPrune_Permissions(t *testing.T) {
	...
	viewerToken, _ := godeployer.GenerateToken("viewer", "viewer", "test-secret-key-12345", 5*time.Second)
...
func TestAPI_SystemPrune_OrphanCleanup(t *testing.T) {
	...
	db, err := godeployer.InitDB("file:mem_prune_1?mode=memory&cache=shared")
	if err != nil {
...
func TestAPI_DiffCache_MaxSizeLimit(t *testing.T) {
	...
	db, err := godeployer.InitDB("file:mem_prune_2?mode=memory&cache=shared")
	if err != nil {
		t.Fatalf("failed to open memory db: %v", err)
	}
	defer db.Close()
