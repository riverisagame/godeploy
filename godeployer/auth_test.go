...
func TestAuth_PasswordHashing(t *testing.T) {
	...
	// 生成 Hash
	hash, err := godeployer.HashPassword(password)
	if err != nil {
	...
	// 比对正确密码
	if !godeployer.CheckPasswordHash(password, hash) {
		t.Error("CheckPasswordHash failed for correct password")
	}

	// 比对错误密码
	if godeployer.CheckPasswordHash("wrongpass", hash) {
...
func TestAuth_TokenLifecycle(t *testing.T) {
	...
	// 签发 Token (有效期 1 秒)
	token, err := godeployer.GenerateToken(username, "admin", secret, 1*time.Second)
	if err != nil {
		t.Fatalf("GenerateToken failed: %v", err)
	}

	// 验证 Token
	parsedUser, parsedRole, err := godeployer.ParseToken(token, secret)
	if err != nil {
	...
	_, _, err = godeployer.ParseToken(token, secret)
	if err == nil {
...
func TestAuth_Middleware(t *testing.T) {
	...
	r := gin.New()
	r.Use(godeployer.AuthMiddleware(secret))
	...
	// 2. 有效 Token 测试
	token, err := godeployer.GenerateToken("john_doe", "admin", secret, 5*time.Second)
	if err != nil {
...
func TestAuth_RoleMiddleware(t *testing.T) {
	...
	r := gin.New()
	r.Use(godeployer.AuthMiddleware(secret))
	r.Use(godeployer.RoleMiddleware("admin", "deployer"))
	r.GET("/protected", func(c *gin.Context) {
		c.String(http.StatusOK, "access granted")
	})

	// 1. 测试 viewer 角色（应被拒绝）
	viewerToken, _ := godeployer.GenerateToken("viewer_user", "viewer", secret, 5*time.Second)
	req, _ := http.NewRequest("GET", "/protected", nil)
	...
	deployerToken, _ := godeployer.GenerateToken("deploy_user", "deployer", secret, 5*time.Second)
