...
type MockNotifier struct {
	mu     sync.Mutex
	events []*godeployer.DeployEvent
}

func (m *MockNotifier) Send(event *godeployer.DeployEvent) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockNotifier) GetEvents() []*godeployer.DeployEvent {
	m.mu.Lock()
	defer m.mu.Unlock()
	copied := make([]*godeployer.DeployEvent, len(m.events))
...
func TestNotifier_AsyncPipeline(t *testing.T) {
	...
	// 抛送事件
	event := &godeployer.DeployEvent{
		Type:     godeployer.EventDeploySuccess,
		TaskId:   1001,
		Project:  "demo",
		Env:      "production",
		Operator: "admin",
	...
	received := events[0]
	if received.TaskId != 1001 || received.Type != godeployer.EventDeploySuccess {
...
func (m *MockFailingNotifier) Send(event *godeployer.DeployEvent) error {
	m.mu.Lock()
	m.CallCount++
...
func TestNotifier_RetryAndDrop(t *testing.T) {
	...
	// 抛送事件
	bus.Publish(&godeployer.DeployEvent{TaskId: 1})
...
func TestNotifier_GracefulShutdown(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		bus.Publish(&godeployer.DeployEvent{TaskId: int64(i)})
	}
	...
	bus.Publish(&godeployer.DeployEvent{TaskId: 999})
}
