...
func TestAPI_BranchDeployDiff_Fallback(t *testing.T) {
	...
	db, err := InitDB(fmt.Sprintf("file:mem_branchdiff_%d?mode=memory&cache=shared", time.Now().UnixNano()))
	if err != nil {
...
// 接口在定位 Git 仓库时不会进入耗时极长的 findGitRepo 磁盘 Walk，而是能被快速安全拦截并优雅降级。
func TestAPI_BranchDeployDiff_NoCache_Fallback(t *testing.T) {
	...
	db, err := InitDB(fmt.Sprintf("file:mem_nocache_%d?mode=memory&cache=shared", time.Now().UnixNano()))
	if err != nil {
		t.Fatalf("failed to init DB: %v", err)
	}
	defer db.Close()
	...
	// 在 workspace 中创建很多无关的目录以测试如果走 findGitRepo 会产生 Walk。
	// 这里通过验证响应时间来证明是否被快速安全拦截，或者验证其是否未执行缓慢的扫描进程。
	// 在未修复时，由于没有 cache 且 buildPath 不存在，且 commit_id 是 master（非Hash），
	// 系统会去走 findGitRepo，对 Workspace 进行 Walk 并查找是否有包含 master 的 git 仓库，这在空目录下可能不会卡死，但我们可以测出结果为“无法获取”。
